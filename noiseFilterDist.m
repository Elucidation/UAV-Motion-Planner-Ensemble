function [filterV sigmaV] = noiseFilterDist()
...
MAX_VARIANCE = 2; % Worst possible error in x,y values (not distance, worst dist is sqrt(2*MAX_VARIANCE^2)
MIN_VARIANCE = 0.0001; % Best possible error in x,y values
POWR = 1.4; % Exponential factor (1+, higher equals faster noise loss)
PERFECT_VIEW = 25; % At how many views till no noise
...
sigmaV = @(viewsCount,distances,k) k*distances.*MAX_VARIANCE.*...
    max(MIN_VARIANCE,PERFECT_VIEW-viewsCount.^POWR+1)./PERFECT_VIEW;

% views must be vector of length numPoints
filterV = @(viewsCount,distances,k) NOISE_MEAN + ...
    sigmaV([viewsCount;viewsCount],[distances;distances],k).*randn(2,size(viewsCount,2));
