function h = plotlocal3d(obs, goal, biggoal, path, closedist, bounds, h)
...
if (biggoal(1) >= minx && biggoal(1) <= maxx && biggoal(2) >= miny && biggoal(2) <= maxy)
    plot3(biggoal(1),biggoal(2),interp2((1:sizex)+minx-1,(1:sizey)+miny-1,cost,biggoal(1),biggoal(2))+.2,'p','MarkerEdgeColor','r','MarkerFaceColor','r','MarkerSize',10);
